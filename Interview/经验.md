...
mongo CDC 读取mongo表再跟用户行为流join，发现内存消耗很大，一共要给70g内存才不会报内存溢出的错误，但总共表也没有多大数据量，

- 后来就去跟底层的数据流向，原来会在snapshot阶段会缓存数据到状态里，然后根据flink的内存管理，这一部分主要属于 manage meomory ，所以需要调大这个manage momoery 的占比。
- 另一方面，我也发现gc次数有很多，通过对gc日志和jstat的分析，换了个G1(有一个humongous区，专门存放短期大对象，减少老年代gc次数。)
- 跟了connector的源码 ，发现在处理document类型时，会。。。
